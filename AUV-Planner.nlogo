...
to setup
  ...
  set Fin one-of p-valids
  ask Fin [set pcolor yellow]
  ...
  ask auvs [ move-to Start ]
  ask dests [ move-to Fin]
...
to voter
  ;; keep track of whether any patch has changed their land
  let any-lands-changed? false
  ask patches
    [ set total (sum [land] of neighbors) ]
  ;; use two ask patches blocks so all patches compute "total"
  ;; before any patches change their lands
  ask patches
    [ let previous-land land
      if total > 5 [ set land 1 ]
      if total < 3 [ set land 0 ]
      if total = 4
      ...
      if total = 3
        [ ifelse award-close-calls-to-loser?
          [ set land 1 ]
          [ set land 0 ] ]
      if land != previous-land
        [ set any-lands-changed? true ]
      recolor-patch ]
  ;; if the lands have stabilized, we stop the simulation
  if not any-lands-changed? [ stop ]
  tick
end
